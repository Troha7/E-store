...
/**
 * {@link CatalogController} is a class for making controller for Thymeleaf.
 *
...
public class CatalogController {
    ...
    public Mono<String> getProducts(@RequestParam(required = false) String name, Model model) {
        ...
        return productsFlux.collectList()
                ...
                .map(products -> "main/catalog")
                .onErrorResume(throwable -> {
                    model.addAttribute("err", throwable.getMessage());
                    return Mono.just("main/catalog");
                });
    }
    ...
    public Mono<String> createProduct(@Validated @ModelAttribute("productDto") ProductRequestDto product, Errors errors) {
        return (errors != null && errors.hasErrors()) ? Mono.just("main/addProduct")
                : productService.create(product).then(Mono.just("redirect:/catalog"));
    }
